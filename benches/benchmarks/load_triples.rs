...
pub fn first90(c: &mut Criterion) {
    c.bench_function("loading 90% of the data", |b| {
	b.iter(|| {
	    black_box({
		timely::execute(timely::Config::process(8), move |worker| {
...
pub fn next9(c: &mut Criterion) {
    c.bench_function("loading 90% + 9% of the data", |b| {
	b.iter(|| {
	    black_box({
		timely::execute(timely::Config::process(8), move |worker| {
...
pub fn next1(c: &mut Criterion) {
    c.bench_function("loading 90% + 9% + 1% of the data", |b| {
	b.iter(|| {
	    black_box({
		timely::execute(timely::Config::process(8), move |worker| {
...
criterion_group!(benches, load_encoded_triples, first90, next9, next1);
