    ...
    G::Timestamp: Lattice,
{
	...
	let (spo, sco) = inn.iterative::<usize,_,_>(|inner| {
		...
		.join_core(&sco_new_arr, |&_, &(_, s), &(p, o_prime)| Some((s, (p, o_prime))));

	    let spo_iter_step = spo_ass_by_o_arr
		.join_core(&spo_new_arr, |&_, &(_, s), &(p, o_prime)| Some((s, (p, o_prime))));
	...
	let property_materialization = inn.iterative::<usize,_,_>(|inner| {
	    ...
	    let spo_type_gen_trans_inv_arr = spo_type_gen_trans_inv_var
		.distinct();

	    let spo_gen_trans_inv_arr = spo_type_gen_trans_inv_arr
		.arrange_by_key();
		...
		.join_core(&spo_gen_trans_inv_arr, |_key, &(_spo, b), &(x, y)| Some((b, (x, y))));

	    let left_inverse_only_iter_step = left_inverse_ass
		.join_core(&spo_gen_trans_inv_arr, |&_, &(_, p1), &(s, o)| Some((p1, (o, s))));

	    let right_inverse_only_iter_step = right_inverse_ass
		.join_core(&spo_gen_trans_inv_arr, |&_, &(_, p0), &(o, s)| Some((p0, (s, o))));
	    
	    let trans_p_only = gen_trans_ass
		.join_core(&spo_gen_trans_inv_arr, |&p, &(_type_kw, _trans_kw), &(s, o)| Some(((s, p), o)));	    
	...
	let domain_type = domain_assertions
	    .join_core(&property_assertions_arr, |_, &(_, x), &(y, _)|Some((4usize, (y, x))));

	let range_assertions = range_assertions.enter(inner);
	
	let range_type = range_assertions
	    .join_core(&property_assertions_arr, |_, &(_, x), &(_, z)|Some((4usize, (z, x))));
