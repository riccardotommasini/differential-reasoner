...
fn tbox_spo_sco_materialization<G>(tbox: &Collection<G, EncodedTriple>, outer: &mut G) -> Collection<G, EncodedTripleByS>
where
    G: Scope,
    G::Timestamp:Lattice
{
	...
	let sco_ass_by_o = tbox_by_o
	    .filter(|(_, (p, _))| p == &0usize);

	let sco_ass_by_s = sco_ass_by_o
	    .map(|(o, (p, s))|((s, (p, o))));
	
	let spo_ass_by_o = tbox_by_o
	    .filter(|(_, (p, _))| p == &1usize);

	let spo_ass_by_s = spo_ass_by_o
	    .map(|(o, (p, s))|((s, (p, o))));
	...
	let (spo, sco) = inn.iterative::<usize,_,_>(|inner| {
	    ...
	    let sco_new_arr = sco_new
		.arrange_by_key();
	    let spo_new_arr = spo_new
		.arrange_by_key();
	    
	    let sco_ass_by_s = sco_ass_by_s
		.enter(inner);
	    let spo_ass_by_s = spo_ass_by_s
		.enter(inner);
	    
	    let sco_ass_by_o = sco_ass_by_o
		.enter(inner);
	    let spo_ass_by_o = spo_ass_by_o
		.enter(inner);
	    
	    let sco_ass_by_o_arr = sco_ass_by_o
		.arrange_by_key();
	    let spo_ass_by_o_arr = spo_ass_by_o
		.arrange_by_key();
	    
	    let sco_iter_step = sco_ass_by_o_arr
		.join_core(&sco_new_arr, |&_, &(p, s), &(_, o_prime)| Some((s, (p, o_prime))));

	    let spo_iter_step = spo_ass_by_o_arr
		.join_core(&spo_new_arr, |&_, &(p, s), &(_, o_prime)| Some((s, (p, o_prime))));
...
fn abox_sco_type_materialization<G>(tbox_sco_assertions: &Collection<G, EncodedTripleByS>,
				    abox_class_assertions: &Collection<G, EncodedTripleByS>,
				    outer: &mut G) -> Collection<G, EncodedTripleByS>
where
    G: Scope,
    G::Timestamp: Lattice,
{
	...
	let class_materialization = inn.iterative::<usize,_,_>(|inner| {
	    ...
	    let sco_type_var = iterate::SemigroupVariable::new(inner, Product::new(Default::default(), 1));

	    let sco_type_new = sco_type_var
		.distinct();

	    let sco_type_arr = sco_type_new
		.arrange_by_key();

	    let class_assertions = class_assertions.enter(inner);

	    let sco_tbox_ass = sco_assertions
		.enter(inner);

	    let sco_iter_step = sco_tbox_ass
		.join_core(&sco_type_arr, |_key, &(_sco, y), &(z, type_)| Some((y, (z, type_))));
...
fn abox_domain_and_range_type_materialization<G>(tbox_domain_assertions: &Collection<G, EncodedTripleByS>,
						 tbox_range_assertions: &Collection<G, EncodedTripleByS>,
						 abox_property_assertions: &Collection<G, EncodedTripleByS>,
						 outer: &mut G) -> (Collection<G, EncodedTripleByS>, Collection<G, EncodedTripleByS>)
where
    G: Scope,
    G::Timestamp: Lattice,
{
	...
	let property_assertions_arr = abox_property_assertions
	    .enter(inner)
	    .arrange_by_key();
	...
	let domain_type = domain_assertions
	    .join_core(&property_assertions_arr, |_, &(_, x), &(y, _)|Some((x, (y, 4usize))));

	let range_assertions = tbox_range_assertions.enter(inner);
	
	let range_type = range_assertions
	    .join_core(&property_assertions_arr, |_, &(_, x), &(_, z)|Some((x, (z, 4usize))));
...
pub fn rdfs<G>(tbox: &Collection<G, EncodedTriple>, abox: &Collection<G, EncodedTriple>, outer: &mut G) -> (Collection<G, EncodedTriple>, Collection<G, EncodedTriple>)
where
    G: Scope,
    G::Timestamp: Lattice,
{
	...
	let property_materialization = inn.iterative::<usize,_,_>(|inner| {
	    ...
	    let spo_type_gen_trans_inv_var = iterate::SemigroupVariable::new(inner, Product::new(Default::default(), 1));
	    
	    let spo_type_gen_trans_inv_new = spo_type_gen_trans_inv_var
		.distinct();

	    let spo_type_gen_trans_inv_arr = spo_type_gen_trans_inv_new
		.arrange_by_key();
		...
		.join_core(&spo_type_gen_trans_inv_arr, |_key, &(_spo, b), &(x, y)| Some((b, (x, y))));
	    
	    spo_type_gen_trans_inv_var
		.set(&property_assertions.enter(inner)
		     .concat(&spo_iter_step));
    ...
    let (domain_type, range_type) = abox_domain_and_range_type_materialization(&domain_assertions, &range_assertions, &property_assertions, outer);

    let class_assertions = class_assertions
        .concatenate(vec![domain_type, range_type])
        .consolidate();

    let class_materialization = abox_sco_type_materialization(&sco_assertions, &class_assertions, outer);

    let class_assertions = class_assertions
        .concat(&class_materialization);
...
pub fn rdfspp<G>(tbox: &Collection<G, EncodedTriple>, abox: &Collection<G, EncodedTriple>, outer: &mut G) -> (Collection<G, EncodedTriple>, Collection<G, EncodedTriple>)
where
    G: Scope,
    G::Timestamp: Lattice,
{
	...
	let property_materialization = inn.iterative::<usize,_,_>(|inner| {
		...
		.enter(inner);
	    let inverse_ass = inverse_of_assertions
		.enter(inner);
		...
		.join_core(&spo_type_gen_trans_inv_arr, |&_, &(_, p1), &(s, o)| Some((p1, (o, s))));

	    let right_inverse_only_iter_step = right_inverse_ass
		.join_core(&spo_type_gen_trans_inv_arr, |&_, &(_, p0), &(o, s)| Some((p0, (s, o))));
	    
	    let trans_p_only = gen_trans_ass
		.join_core(&spo_type_gen_trans_inv_arr, |&p, &(_type_kw, _trans_kw), &(s, o)| Some(((s, p), o)));
		...
		.join_core(&trans_p_only_arr, |&(_o, p), &s, &o_prime| Some((p, (s, o_prime))));
