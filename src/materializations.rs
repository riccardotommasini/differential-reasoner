    ...
    G::Timestamp: Lattice,
{
	...
	let (sco_type, spo_type) = inn.iterative::<usize,_,_>(|inner| {
		...
		.join_core(&sco_arr, |_key, &(_sco, y), &(z, type_)| Some((y, (z, type_))));

	    let spo_iter_step = spo_ass
		.join_core(&spo_gen_trans_arr, |_key, &(_spo, b), &(x, y)| Some((b, (x, y))));
	...
	let domain_type = domain_assertions
	    .join_core(&not_type_assertions_by_p_arr, |_a, &(_domain, x), &(y, _z)|Some((4usize, (y, x))));

	let range_assertions = range_assertions.enter(inner);
	
	let range_type = range_assertions
	    .join_core(&not_type_assertions_by_p_arr, |_a, &(_range, x), &(_y, z)|Some((4usize, (z, x))));
